...
Private Const CHART_NAME_TIMELINE As String = "chTimeline_FOS"

Private Type FOSLayout
...
' CACHE
Private fosLoaded As Boolean
Private fosN As Long, fosM As Long, fosMaxOps As Long, fosTotalOps As Long, fosMaxDecCols As Long, fosTotalMach As Long
Private fosJobs() As String, fosR() As Double, fosD() As Double, fosW() As Double
Private fosCk() As Long, fosRmaq() As Double
Private fosOpJob() As Long, fosOpNum() As Long, fosOpWS() As Long, fosOpP() As Double, fosOpS() As Double
Private fosOpsPerJob() As Long, fosOpIdx() As Long, fosOpsOnWS() As Long
Private fosMachSeq() As Long
Private fosOpsOnMach() As Long
Private fosOpAssign() As Long
Private fosGlobalStart() As Long

' =========================================================
Public Sub RedibujarInputs_FOS(ByVal ws As Worksheet)
    On Error GoTo ErrH
    Dim n As Long, m As Long, maxOps As Long
    If Not FOS_ReadNMO(ws, n, m, maxOps) Then Exit Sub

    ws.Unprotect
    fosLoaded = False: fosN = 0: fosM = 0: fosMaxOps = 0: fosTotalOps = 0
    ws.Range(CELL_FOS_READY).Value = ""
    FOS_ClearDynamicZone ws: FOS_DeleteChartIfExists ws

    Dim totalMach As Long: totalMach = m
    Dim maxDecCols As Long: maxDecCols = n * maxOps
    Dim L As FOSLayout: FOS_GetLayout n, m, maxOps, totalMach, maxDecCols, L

    ' Tabla 1
    FOS_DrawSectionTitle ws, L.pTitleRow, P_COL_JOB, "2. Parámetros por job [PARÁMETRO] (escribe r, d y w)."
    FOS_DrawHeaderRow ws, L.pHeaderRow, P_COL_JOB, P_COL_W, Array("Job", "r", "d", "w")
    FOS_DrawEditableBlock ws, L.pFirstRow, L.pLastRow, P_COL_JOB, P_COL_W
    Dim i As Long
    For i = 1 To n
    ...
    FOS_SetJobListNamedRange ws, L.pFirstRow, L.pLastRow

    ' Tabla 2
    FOS_DrawSectionTitle ws, L.wsTitleRow, WS_COL_LABEL, "3. Workstations [PARÁMETRO] — # máquinas idénticas y rmaq por workstation. Al cambiar c, la Tabla 7 se reajusta."
    FOS_DrawHeaderRow ws, L.wsHeaderRow, WS_COL_LABEL, WS_COL_RMAQ, Array("Workstation", "c (# máquinas)", "rmaq")
    FOS_DrawEditableBlock ws, L.wsFirstRow, L.wsLastRow, WS_COL_LABEL, WS_COL_RMAQ
    Dim k As Long
    For k = 1 To m
    ...
    FOS_DrawSectionTitle ws, L.pmatTitleRow, MAT_COL_LABEL, "4. Tiempo de procesamiento p [PARÁMETRO]."
    FOS_DrawMatrix ws, L.pmatHeaderRow, L.pmatFirstRow, L.pmatLastRow, n, maxOps, "Job", "Op", True, RGB(248, 248, 248)
    FOS_PrellenarLabelsJobs ws, L.pmatFirstRow, n

    FOS_DrawSectionTitle ws, L.smatTitleRow, MAT_COL_LABEL, "5. Tiempo de setup s [PARÁMETRO]."
    FOS_DrawMatrix ws, L.smatHeaderRow, L.smatFirstRow, L.smatLastRow, n, maxOps, "Job", "Op", True, RGB(248, 248, 248)
    FOS_PrellenarLabelsJobs ws, L.smatFirstRow, n

    ' Tabla Ruta — DECISIÓN
    FOS_DrawSectionTitle ws, L.routeTitleRow, MAT_COL_LABEL, "6. Ruta [DECISIÓN] — escribe el workstation que visita cada job en cada operación."
    With ws.Cells(L.routeHelpRow, MAT_COL_LABEL)
        .Value = "Elige el workstation para cada operación. Se permite repetir. Deja vacías las ops que no use el job (sin huecos)."
        .Font.Italic = True: .HorizontalAlignment = xlLeft: .VerticalAlignment = xlCenter: .Locked = True
    End With
    FOS_DrawMatrix ws, L.routeHeaderRow, L.routeFirstRow, L.routeLastRow, n, maxOps, "Job", "Op", False, 0
    FOS_PrellenarLabelsJobs ws, L.routeFirstRow, n
    FOS_ApplyWorkstationDropdownMatrix ws, m, L.routeFirstRow, L.routeLastRow, MAT_COL_FIRST, MAT_COL_FIRST + maxOps - 1

    Dim ck() As Long: ReDim ck(1 To m)
    For k = 1 To m: ck(k) = 1: Next k
    FOS_DrawDecisionTable ws, L, n, m, maxOps, ck
...
Salir:
    ...
    Exit Sub
ErrH:
    MsgBox "Error en RedibujarInputs_FOS: " & Err.Description, vbExclamation: Resume Salir
End Sub

Public Sub FOS_RebuildDecisionTable(ByVal ws As Worksheet)
    On Error GoTo ErrH
    Dim n As Long, m As Long, maxOps As Long
    If Not FOS_ReadNMO(ws, n, m, maxOps) Then Exit Sub
    ws.Unprotect
    ...
    Dim L0 As FOSLayout: FOS_GetLayout n, m, maxOps, m, n * maxOps, L0
    Dim ck() As Long: ReDim ck(1 To m)
    ...
    Dim L As FOSLayout: FOS_GetLayout n, m, maxOps, total, n * maxOps, L
...
Salir:
    ...
    Exit Sub
ErrH:
    MsgBox "Error en FOS_RebuildDecisionTable: " & Err.Description, vbExclamation: Resume Salir
End Sub

Public Function FOS_CkRange(ByVal ws As Worksheet) As Range
    Dim n As Long, m As Long, maxOps As Long
    If Not FOS_ReadNMO(ws, n, m, maxOps) Then Exit Function
    Dim L As FOSLayout: FOS_GetLayout n, m, maxOps, m, n * maxOps, L
    Set FOS_CkRange = ws.Range(ws.Cells(L.wsFirstRow, WS_COL_C), ws.Cells(L.wsLastRow, WS_COL_C))
End Function

Public Sub FOS_CargarDatos()
    On Error GoTo ErrH
    Dim ws As Worksheet: Set ws = ThisWorkbook.Worksheets(SHEET_NAME)
    Dim n As Long, m As Long, maxOps As Long
    If Not FOS_ReadNMO(ws, n, m, maxOps) Then
    ...
    Dim L0 As FOSLayout: FOS_GetLayout n, m, maxOps, m, n * maxOps, L0
    If Not FOS_ReadAndValidateJobTable(ws, n, L0, jobs, rJobArr, d, w, warn) Then
        MsgBox warn, vbExclamation, "Revisar datos": GoTo SalirFail
    End If
    Dim ck() As Long, rmaq() As Double
    If Not FOS_ReadAndValidateWorkstations(ws, m, L0, ck, rmaq, warn) Then
        MsgBox warn, vbExclamation, "Revisar datos": GoTo SalirFail
    End If
    ...
    Dim maxDecCols As Long: maxDecCols = n * maxOps
    Dim L As FOSLayout: FOS_GetLayout n, m, maxOps, totalMach, maxDecCols, L
    FOS_ClearOutputArea ws, L: FOS_DeleteChartIfExists ws
    ws.Range(CELL_FOS_READY).Value = ""
    ...
    If Not FOS_ReadAndValidateRouteMatrices(ws, n, m, maxOps, L, opJob, opNum, opWS, opP, opS, _
                                            opsPerJob, opsOnWS, opIdxMat, totalOps, warn) Then
        MsgBox warn, vbExclamation, "Revisar datos": GoTo SalirFail
    End If

    Dim machSeq() As Long, opsOnMach() As Long, opAssign() As Long, globalStart() As Long
    If Not FOS_ReadAndValidateDecisionMatrix(ws, n, m, maxOps, maxDecCols, totalMach, ck, L, _
    ...
    FOS_StoreInCache n, m, maxOps, totalOps, maxDecCols, totalMach, jobs, rJobArr, d, w, ck, rmaq, _
    ...
    If Not FOS_ScheduleFromCache(st, ct, Cmax, warn) Then
        MsgBox warn, vbExclamation, "Revisar decisión": fosLoaded = False: GoTo SalirFail
    End If

    fosLoaded = True
    ws.Range(CELL_FOS_READY).Value = "OK"
...
Salir:
    ...
    Exit Sub
SalirFail:
    fosLoaded = False: GoTo Salir
ErrH:
    MsgBox "Error en FOS_CargarDatos: " & Err.Description, vbExclamation: Resume Salir
End Sub

Public Sub FOS_GenerarOutputs()
    On Error GoTo ErrH
    Dim ws As Worksheet: Set ws = ThisWorkbook.Worksheets(SHEET_NAME)
    If UCase$(Trim$(CStr(ws.Range(CELL_FOS_READY).Value))) <> "OK" Then
        MsgBox "Primero presione 'Cargar datos'.", vbExclamation: Exit Sub
    End If
    Dim n As Long, m As Long, maxOps As Long
    If Not FOS_ReadNMO(ws, n, m, maxOps) Then Exit Sub
    If Not fosLoaded Or fosN <> n Or fosM <> m Or fosMaxOps <> maxOps Then
        MsgBox "Presione 'Cargar datos' nuevamente.", vbExclamation: Exit Sub
    End If

    ws.Unprotect
    Dim L As FOSLayout: FOS_GetLayout n, m, maxOps, fosTotalMach, fosMaxDecCols, L
    FOS_DeleteChartIfExists ws: FOS_ClearOutputArea ws, L
    ...
    If Not FOS_ScheduleFromCache(st, ct, Cmax, warn) Then MsgBox warn, vbExclamation: GoTo Salir
    ...
    FOS_ComputeJobMetrics st, ct, startJob, Cj, Flow, Lj, Tard, wT, avgFlow, Lmax, avgT, sumWT, lateCount, pctLate, pctOnTime

    Dim chObj As ChartObject
    Set chObj = ws.ChartObjects.Add(Left:=ws.Cells(ganttTopRow, 2).Left, top:=ws.Cells(ganttTopRow, 2).top, _
        Width:=1100, Height:=240 + 22 * fosTotalMach)
    chObj.name = CHART_NAME_TIMELINE
    With chObj.Chart
        .ChartType = xlBarStacked: .HasTitle = True: .ChartTitle.text = "Gantt Flexible Open Shop": .HasLegend = False
        Do While .SeriesCollection.Count > 0: .SeriesCollection(1).Delete: Loop
    End With

    FOS_DrawIndicatorsStructure ws, indTopRow
    Dim jmFirstCol As Long: jmFirstCol = 8
    Dim jmFirstDataRow As Long
    FOS_DrawJobMetricsTable ws, n, indTopRow, jmFirstCol, jmFirstDataRow
    ...
    For j = 1 To n
        machStr = ""
        For op = 1 To fosOpsPerJob(j)
            opIdx = fosOpIdx(j, op)
            machStr = machStr & "M" & fosOpWS(opIdx) & "." & fosOpAssign(opIdx)
            If op < fosOpsPerJob(j) Then machStr = machStr & ", "
        Next op
        ws.Cells(jmFirstDataRow + j - 1, jmFirstCol).Value = fosJobs(j)
    ...
    FOS_BuildGantt chObj.Chart, st, ct, Cmax
...
Salir:
    ...
    Exit Sub
ErrH:
    MsgBox "Error en FOS_GenerarOutputs: " & Err.Description, vbExclamation: Resume Salir
End Sub

Private Sub FOS_GetLayout(ByVal n As Long, ByVal m As Long, ByVal maxOps As Long, _
        ByVal totalMach As Long, ByVal maxDecCols As Long, ByRef L As FOSLayout)
    ...
    ' Parámetros (p, s) antes que las tablas de decisión
    L.pmatTitleRow = L.wsLastRow + 3: L.pmatHeaderRow = L.pmatTitleRow + 1
    ...
    ' Decisiones (ruta + secuencia por máquina individual)
    L.routeTitleRow = L.smatLastRow + 3: L.routeHelpRow = L.routeTitleRow + 1: L.routeHeaderRow = L.routeHelpRow + 1
    L.routeFirstRow = L.routeHeaderRow + 1: L.routeLastRow = L.routeFirstRow + n - 1
    L.decTitleRow = L.routeLastRow + 3: L.decHeaderRow = L.decTitleRow + 1
...
Public Function FOS_ReadNMO(ByVal ws As Worksheet, ByRef n As Long, ByRef m As Long, ByRef maxOps As Long) As Boolean
    FOS_ReadNMO = False
...
Private Sub FOS_DrawDecisionTable(ByVal ws As Worksheet, ByRef L As FOSLayout, _
    ...
    FOS_DrawSectionTitle ws, L.decTitleRow, MAT_COL_LABEL, "7. Secuencia por máquina individual [DECISIÓN] — orden de jobs en cada máquina específica."
    FOS_DrawMatrix ws, L.decHeaderRow, L.decFirstRow, L.decLastRow, totalMach, maxDecCols, "Máquina", "Pos", False, 0
    ...
    FOS_ApplyJobDropdownMatrix ws, L.decFirstRow, L.decLastRow, MAT_COL_FIRST, MAT_COL_FIRST + maxDecCols - 1
End Sub

Private Function FOS_ReadAndValidateJobTable(ByVal ws As Worksheet, ByVal n As Long, ByRef L As FOSLayout, _
        ByRef jobs() As String, ByRef rJobArr() As Double, ByRef d() As Double, ByRef w() As Double, ByRef warn As String) As Boolean
    FOS_ReadAndValidateJobTable = False: warn = ""
    ...
    Next i
    FOS_ReadAndValidateJobTable = True
End Function

Private Function FOS_ReadAndValidateWorkstations(ByVal ws As Worksheet, ByVal m As Long, ByRef L As FOSLayout, _
        ByRef ck() As Long, ByRef rmaq() As Double, ByRef warn As String) As Boolean
    FOS_ReadAndValidateWorkstations = False: warn = "": ReDim ck(1 To m): ReDim rmaq(1 To m)
    ...
    For k = 1 To m
        ...
        rmaq(k) = CDbl(rv)
    Next k
    FOS_ReadAndValidateWorkstations = True
End Function

Private Function FOS_ReadAndValidateRouteMatrices(ByVal ws As Worksheet, ByVal n As Long, ByVal m As Long, ByVal maxOps As Long, _
        ByRef L As FOSLayout, ByRef opJob() As Long, ByRef opNum() As Long, ByRef opWS() As Long, _
    ...
    FOS_ReadAndValidateRouteMatrices = False: warn = ""
    ...
    For j = 1 To n
        ...
        For k = 1 To maxOps
            ...
            Else
                ...
                If Not FOS_IsValidWS(wsStr, m) Then warn = "Workstation inválido '" & wsStr & "' en J" & j & " Op " & k & ".": Exit Function
                wsIdx = FOS_WSIndex(wsStr)
    ...
    For j = 1 To n
        ...
        Next k
    Next j
    FOS_ReadAndValidateRouteMatrices = True
End Function

Private Function FOS_ReadAndValidateDecisionMatrix(ByVal ws As Worksheet, _
        ByVal n As Long, ByVal m As Long, ByVal maxOps As Long, ByVal maxDecCols As Long, _
        ByVal totalMach As Long, ByRef ck() As Long, ByRef L As FOSLayout, _
    ...
    FOS_ReadAndValidateDecisionMatrix = False: warn = ""
        ...
        For mi = 1 To ck(k)
            ...
            For pos = 1 To maxDecCols
                ...
                Else
                    ...
                    Dim opsInWS As Long: opsInWS = UBound(parts)
    ...
    Next k
    FOS_ReadAndValidateDecisionMatrix = True
End Function

Private Sub FOS_StoreInCache(ByVal n As Long, ByVal m As Long, ByVal maxOps As Long, ByVal totalOps As Long, _
    ...
    fosLoaded = False
    fosN = n: fosM = m: fosMaxOps = maxOps: fosTotalOps = totalOps
    fosMaxDecCols = maxDecCols: fosTotalMach = totalMach
    ReDim fosJobs(1 To n): ReDim fosR(1 To n): ReDim fosD(1 To n): ReDim fosW(1 To n)
    ReDim fosCk(1 To m): ReDim fosRmaq(1 To m): ReDim fosGlobalStart(1 To m)
    ReDim fosOpJob(1 To totalOps): ReDim fosOpNum(1 To totalOps): ReDim fosOpWS(1 To totalOps)
    ReDim fosOpP(1 To totalOps): ReDim fosOpS(1 To totalOps): ReDim fosOpAssign(1 To totalOps)
    ReDim fosOpsPerJob(1 To n): ReDim fosOpsOnWS(1 To m)
    ReDim fosOpIdx(1 To n, 1 To maxOps)
    ReDim fosMachSeq(1 To totalMach, 1 To maxDecCols): ReDim fosOpsOnMach(1 To totalMach)
    Dim i As Long, k As Long
    For i = 1 To n: fosJobs(i) = jobs(i): fosR(i) = rJobArr(i): fosD(i) = d(i): fosW(i) = w(i): fosOpsPerJob(i) = opsPerJob(i): Next i
    For k = 1 To m: fosCk(k) = ck(k): fosRmaq(k) = rmaq(k): fosOpsOnWS(k) = opsOnWS(k): fosGlobalStart(k) = globalStart(k): Next k
    For i = 1 To totalOps
        fosOpJob(i) = opJob(i): fosOpNum(i) = opNum(i): fosOpWS(i) = opWS(i)
        fosOpP(i) = opP(i): fosOpS(i) = opS(i): fosOpAssign(i) = opAssign(i)
    Next i
    For i = 1 To n: For k = 1 To maxOps: fosOpIdx(i, k) = opIdxMat(i, k): Next k: Next i
    For i = 1 To totalMach
        fosOpsOnMach(i) = opsOnMach(i)
        Dim pos As Long: For pos = 1 To maxDecCols: fosMachSeq(i, pos) = machSeq(i, pos): Next pos
    Next i
End Sub

Private Function FOS_ScheduleFromCache(ByRef st() As Double, ByRef ct() As Double, ByRef Cmax As Double, ByRef warn As String) As Boolean
    FOS_ScheduleFromCache = False: warn = "": Cmax = 0#
    Dim totalOps As Long: totalOps = fosTotalOps
    Dim n As Long: n = fosN: Dim m As Long: m = fosM: Dim totalMach As Long: totalMach = fosTotalMach
    ...
    For j = 1 To n
        For op = 2 To fosOpsPerJob(j)
            u = fosOpIdx(j, op - 1): v = fosOpIdx(j, op)
            FOS_AddEdge u, v, head, toN, nxt, indeg, eCount
        Next op
    Next j
    For g = 1 To totalMach
        For pos = 2 To fosOpsOnMach(g)
            u = fosMachSeq(g, pos - 1): v = fosMachSeq(g, pos)
            FOS_AddEdge u, v, head, toN, nxt, indeg, eCount
        Next pos
    Next g
    ...
    For k = 1 To m
        For i = 1 To fosCk(k)
            machFree(fosGlobalStart(k) + i) = fosRmaq(k)
        Next i
    Next k

    Dim est() As Double: ReDim est(1 To totalOps)
    For i = 1 To totalOps: est(i) = fosR(fosOpJob(i)): Next i
    ...
    Do While qh <= qt
        ...
        gIdx = fosGlobalStart(fosOpWS(u)) + fosOpAssign(u)
        startU = est(u)
        If machFree(gIdx) > startU Then startU = machFree(gIdx)
        st(u) = startU
        ct(u) = startU + fosOpS(u) + fosOpP(u)
        machFree(gIdx) = ct(u)
    ...
    FOS_ScheduleFromCache = True
End Function

Private Sub FOS_AddEdge(ByVal u As Long, ByVal v As Long, ByRef head() As Long, ByRef toN() As Long, _
...
Private Sub FOS_ComputeJobMetrics(ByRef st() As Double, ByRef ct() As Double, _
    ...
    Dim n As Long: n = fosN: Dim totalOps As Long: totalOps = fosTotalOps
    ...
    For j = 1 To n
        ...
        Flow(j) = Cj(j) - fosR(j): Lj(j) = Cj(j) - fosD(j)
...
Private Sub FOS_BuildGantt(ByVal ch As Chart, ByRef st() As Double, ByRef ct() As Double, ByVal Cmax As Double)
    Dim m As Long: m = fosM: Dim totalOps As Long: totalOps = fosTotalOps
    Dim total As Long: total = fosTotalMach
    ...
    With ch
        ...
        FOS_SortIdxByMachStart totalOps, idx, st
        ...
        For ii = 1 To totalOps
            i = idx(ii)
            mIdx = fosGlobalStart(fosOpWS(i)) + fosOpAssign(i)
            setupDur = fosOpS(i): procDur = fosOpP(i)
            gap = st(i) - machOff(mIdx)
            If gap > 0.000001 Then
                Set srs = .SeriesCollection.NewSeries
                srs.Values = FOS_OneHot(total, mIdx, gap): srs.XValues = cats
                srs.Format.Fill.Visible = msoFalse: srs.Format.Line.Visible = msoFalse
                machOff(mIdx) = machOff(mIdx) + gap
            End If
            baseCol = FOS_ColorForJobName(fosJobs(fosOpJob(i)), jobColor)
            If setupDur > 0.000001 Then
                Set srs = .SeriesCollection.NewSeries
                srs.Values = FOS_OneHot(total, mIdx, setupDur): srs.XValues = cats
                srs.Format.Fill.ForeColor.RGB = FOS_LightTone(baseCol): srs.Format.Line.Visible = msoFalse
                machOff(mIdx) = machOff(mIdx) + setupDur
            End If
            If procDur > 0.000001 Then
                Set srs = .SeriesCollection.NewSeries
                srs.Values = FOS_OneHot(total, mIdx, procDur): srs.XValues = cats
                srs.Format.Fill.ForeColor.RGB = FOS_DarkTone(baseCol): srs.Format.Line.Visible = msoFalse
                With srs.Points(mIdx)
                    .HasDataLabel = True: .DataLabel.text = fosJobs(fosOpJob(i)) & " (Op " & fosOpNum(i) & ")": .DataLabel.Font.Size = 9
                End With
        ...
        FOS_ConfigurarEjeTiempo ch, Cmax
    End With
End Sub

Private Sub FOS_SortIdxByMachStart(ByVal n As Long, ByRef idx() As Long, ByRef st() As Double)
    Dim i As Long, j As Long, t As Long, ai As Long, aj As Long
    For i = 1 To n - 1
        For j = i + 1 To n
            ai = fosGlobalStart(fosOpWS(idx(i))) + fosOpAssign(idx(i))
            aj = fosGlobalStart(fosOpWS(idx(j))) + fosOpAssign(idx(j))
...
Private Sub FOS_DrawSectionTitle(ByVal ws As Worksheet, ByVal rowN As Long, ByVal col As Long, ByVal txt As String)
    ...
    End With
End Sub
Private Sub FOS_DrawHeaderRow(ByVal ws As Worksheet, ByVal rowN As Long, ByVal col1 As Long, ByVal col2 As Long, ByVal labels As Variant)
    ...
    End With
End Sub
Private Sub FOS_DrawEditableBlock(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long, ByVal col1 As Long, ByVal col2 As Long)
    ...
    End With
End Sub
Private Sub FOS_DrawMatrix(ByVal ws As Worksheet, ByVal headerRow As Long, ByVal firstRow As Long, ByVal lastRow As Long, _
...
End Sub
Private Sub FOS_PrellenarLabelsJobs(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal n As Long)
    Dim i As Long
    For i = 1 To n
    ...
    Next i
End Sub
Private Sub FOS_DrawIndicatorsStructure(ByVal ws As Worksheet, ByVal indTopRow As Long)
    ...
    End With
End Sub
Private Sub FOS_DrawJobMetricsTable(ByVal ws As Worksheet, ByVal n As Long, ByVal indTopRow As Long, ByVal firstCol As Long, ByRef firstDataRow As Long)
...
Private Sub FOS_ClearDynamicZone(ByVal ws As Worksheet)
    ...
    End With
End Sub
Private Sub FOS_ClearOutputArea(ByVal ws As Worksheet, ByRef L As FOSLayout)
    ...
    End With
End Sub
Private Sub FOS_DeleteChartIfExists(ByVal ws As Worksheet)
    Dim i As Long: On Error Resume Next
    For i = ws.ChartObjects.Count To 1 Step -1: ws.ChartObjects(i).Delete: Next i
    On Error GoTo 0
End Sub
Private Sub FOS_ApplyWorkstationDropdownMatrix(ByVal ws As Worksheet, ByVal m As Long, ByVal fr As Long, ByVal lR As Long, ByVal fC As Long, ByVal lC As Long)
    ...
    End With
End Sub
Private Sub FOS_ApplyJobDropdownMatrix(ByVal ws As Worksheet, ByVal fr As Long, ByVal lR As Long, ByVal fC As Long, ByVal lC As Long)
    With ws.Range(ws.Cells(fr, fC), ws.Cells(lR, lC)).Validation
        .Delete: .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:="=FOS_JobList"
        .IgnoreBlank = True: .InCellDropdown = True
    End With
End Sub
Private Sub FOS_SetJobListNamedRange(ByVal ws As Worksheet, ByVal firstRow As Long, ByVal lastRow As Long)
    On Error Resume Next: ws.Parent.Names("FOS_JobList").Delete: On Error GoTo 0
    ws.Parent.Names.Add name:="FOS_JobList", _
        RefersTo:=ws.Range(ws.Cells(firstRow, P_COL_JOB), ws.Cells(lastRow, P_COL_JOB))
End Sub
Private Function FOS_IsValidWS(ByVal s As String, ByVal m As Long) As Boolean
    s = UCase$(Trim$(s)): If Len(s) < 2 Then Exit Function: If Left$(s, 1) <> "M" Then Exit Function
    FOS_IsValidWS = (val(Mid$(s, 2)) >= 1 And val(Mid$(s, 2)) <= m)
End Function
Private Function FOS_WSIndex(ByVal s As String) As Long
    s = UCase$(Trim$(s)): FOS_WSIndex = val(Mid$(s, 2)): If FOS_WSIndex < 1 Then FOS_WSIndex = 1
End Function
Private Function FOS_OneHot(ByVal total As Long, ByVal idx As Long, ByVal v As Double) As Variant
    Dim a() As Double, t As Long: ReDim a(1 To total)
    For t = 1 To total: a(t) = 0#: Next t
    a(idx) = v: FOS_OneHot = a
End Function
Private Function FOS_ColorForJobName(ByVal jn As String, ByVal jc As Object) As Long
    Dim k As String: k = UCase$(Trim$(jn))
    If Not jc.Exists(k) Then jc.Add k, jc.Count + 1
    FOS_ColorForJobName = FOS_BasePalette(jc(k))
End Function
Private Function FOS_BasePalette(ByVal i As Long) As Long
    Dim p As Variant
    ...
    FOS_BasePalette = p((i - 1) Mod (UBound(p) + 1))
End Function
Private Function FOS_LightTone(ByVal c As Long) As Long
    Dim r As Long, g As Long, b As Long
    r = (c And &HFF): g = (c \ &H100) And &HFF: b = (c \ &H10000) And &HFF
    FOS_LightTone = RGB(IIf(r + 80 < 255, r + 80, 255), IIf(g + 80 < 255, g + 80, 255), IIf(b + 80 < 255, b + 80, 255))
End Function
Private Function FOS_DarkTone(ByVal c As Long) As Long
    Dim r As Long, g As Long, b As Long
    r = (c And &HFF): g = (c \ &H100) And &HFF: b = (c \ &H10000) And &HFF
    FOS_DarkTone = RGB(IIf(r > 40, r - 40, 0), IIf(g > 40, g - 40, 0), IIf(b > 40, b - 40, 0))
End Function
Private Sub FOS_ConfigurarEjeTiempo(ByVal ch As Chart, ByVal maxTime As Double)
    Dim eje As Axis: Set eje = ch.Axes(xlValue)
    If maxTime < 0.000001 Then maxTime = 1
    Dim majorU As Double: majorU = FOS_NiceMajorUnit(maxTime, 30)
    ...
    On Error GoTo 0
End Sub
Private Function FOS_NiceMajorUnit(ByVal mx As Double, ByVal tt As Long) As Double
    If mx <= 0 Then FOS_NiceMajorUnit = 1: Exit Function
    Dim rs As Double: rs = mx / tt: If rs < 1 Then rs = 1
    Dim p10 As Double: p10 = 10 ^ Int(Log(rs) / Log(10))
    Dim fr As Double: fr = rs / p10
    Select Case fr
        Case Is <= 1: FOS_NiceMajorUnit = 1 * p10
        Case Is <= 2: FOS_NiceMajorUnit = 2 * p10
        Case Is <= 5: FOS_NiceMajorUnit = 5 * p10
        Case Else: FOS_NiceMajorUnit = 10 * p10
    End Select
End Function
